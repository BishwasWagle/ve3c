...
bool client_application(secure_authenticated_channel &channel, const string &channel_str) {
  ...
  string msg = "Hi from " + channel.peer_id_+channel_str;
...
void server_application(secure_authenticated_channel &channel) {
  ...
  // Differentiate clients based on peer_id_ or received message
  string response;
  if (out.find("client1") != string::npos) {
    response = "Hello Client 1! Your message was received securely.\n";
  } else if (out.find("client2") != string::npos) {
    response = "Hello Client 2! Welcome to the secure server.\n";
  } else {
    response = "Unknown client connected.\n";
  }
   // Reply over authenticated, encrypted channel
  channel.write(response.size(), (byte *)response.c_str());
  channel.close();
}
...
int main(int an, char **av) {
  ...
  if (FLAGS_operation == "cold-init") {
    ...
    trust_mgr->print_trust_data();
#endif  // DEBUG
  } else if (FLAGS_operation == "run-app-as-client-1") {
    string                       my_role("client1");
    ...
    if (!client_application(channel, "client1")) {
  ...
  }else if (FLAGS_operation == "run-app-as-client-2") {
    string                       my_role("client2");
    secure_authenticated_channel channel(my_role);
    ...
    // This is the actual application code.
    if (!client_application(channel, "client2")) {
      printf("%s() error, line %d, client_application failed\n",
             __func__,
             __LINE__);
      ret = 1;
